...
class MerchantList extends Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      ...
      if (searchParams && searchParams.merchantList) {
        this.setState({
          currentPage: searchParams.merchantList.fromRow / searchParams.merchantList.pageSize + 1,
          perPage: searchParams.merchantList.pageSize,
        });
      }
    }, 0);
  }
  ...
  handlePageChange = (pageNumber) => {
    ...
    if (
      ...
      this.setState({
        ...
        storedData = {
          ...(storedData && storedData.merchantList) || defaultData,
    ...
    return (
      <>
        <div className="animated fadeIn fee-table" style={{ background: 'white' }}>
          <Card>
            <CardHeader>
              <span className="text-bold">{Constants.MerchantList.merchantList}</span>
            </CardHeader>
            <CardBody>
              <FillMerchant
              ...
              />
              <TableMerchant
                tData={listMerchantTable && listMerchantTable.merchantList}
                perPage={perPage}
                currentPage={currentPage}
              />
