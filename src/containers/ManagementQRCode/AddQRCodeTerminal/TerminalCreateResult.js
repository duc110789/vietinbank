...
class TerminalCreateResult extends Component {
  constructor(props) {
    super(props);
    this.state = {
      qrCodeDetail: {},
  ...
  componentDidMount() {
    ...
    if (localStorage.getItem('ID_QRCODE_TERMINAL')) {
      getQrCode({
        qrcodeId: JSON.parse(window.atob(localStorage.getItem('ID_QRCODE_TERMINAL'))),
      });
    }
  }
                  ...
                  <Col className="col-md-4 col-12">
                    <FormGroup row className="text-center btn-search">
                      ...
                      <Col lg="12" className="mb-2">
                        <Button className="icon-search btn btn-primary btn btn-secondary w-75" onClick={() => this.downloadImageQr()}>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            {
                            (qrCodeDetail && qrCodeDetail.data
                              && qrCodeDetail.data.merchantCode) || 0
                          }
                          </td>
                        </tr>
                        <tr>
          ...
          <Row>
            ...
            <Col md="6" className="text-left">
              ...
              <p className="qr-download">
                Mã sản phẩm:
                {' '}
                {qrCodeDetail && qrCodeDetail.data && qrCodeDetail.data.productID ? qrCodeDetail.data.productID : ''}
              </p>
              <p className="qr-download">
                Tên sản phẩm:
                {' '}
                {qrCodeDetail && qrCodeDetail.data && qrCodeDetail.data.productName ? qrCodeDetail.data.productName : ''}
              </p>
              <p className="qr-download">
                Giá tiền:
                {' '}
                {qrCodeDetail && qrCodeDetail.data && qrCodeDetail.data.amount ? numberWithDots(qrCodeDetail.data.amount) : 0}
                {' '}
                VNĐ
              </p>
            </Col>
...
TerminalCreateResult.defaultProps = {
  history: {},
  getQrCode: () => {},
};

TerminalCreateResult.propTypes = {
  history: PropTypes.object,
  getQrCode: PropTypes.object,
};

export default connect(mapStateToProps, mapDispatchToProps)(withRouter(TerminalCreateResult));
