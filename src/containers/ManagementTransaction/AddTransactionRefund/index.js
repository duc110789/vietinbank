...
import {checkLength6To150, checkRequiredSelectInput} from '../../../components/Input/validation';
import Constants from '../Constants';
import {createTransactionRefund, getTransactionDetail} from '../../../store/actions/ManagementTransaction/index';
import {messageError, messageSuccess} from '../../../utils';
import {numberWithDots, renderTransactionStatusColor} from '../../../utils/commonFunction';

class AddTransitionRefund extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isOpenInfo: true,
      ...
      validateForm: false,
      checkInitAmountSmallerDebitAmount: false,
      description: '',
      typeRefund: 1,
      initAmount: 0,
    };
  ...
  componentDidUpdate(prevProps) {
    const { responseServer, history, response } = this.props;
    if (responseServer !== prevProps.responseServer) {
      if (responseServer.code !== '00') {
        messageError(responseServer.description);
      } else if (responseServer.description === 'Success') {
        messageSuccess('Khởi tạo yêu cầu hoàn tiền thành công');
        history.push({
  ...
  onChangeTypeRefund = (e) => {
    this.setState({
      typeRefund: e.value,
    });
  };

  handleChangeMoneyRequiredRefund = (e) => {
    const { transactionDetail } = this.props;
    if (((e.target.value.replace(/,/g, '') - (transactionDetail && parseInt(transactionDetail.debitAmount, 10))) < 0)) {
      this.setState({
        initAmount: parseInt(e.target.value.replace(/,/g, ''), 10),
        checkInitAmountSmallerDebitAmount: false,
      });
    } else {
      this.setState({
        checkInitAmountSmallerDebitAmount: true,
      });
    }
  };
  ...
  createRefundTransaction = () => {
    ...
    if (
      ...
      const data = {
        id: JSON.parse(window.atob(window.localStorage.getItem('ID_TRANSACTION'))),
        localDate: JSON.parse(window.atob(window.localStorage.getItem('LOCAL_DATE_TRANSACTION_DETAIL'))),
        typeRefund: (transactionDetail && transactionDetail.refundBeans
          && (transactionDetail.refundBeans.length > 0)) ? '2' : typeRefund,
        initAmount:
          (
            transactionDetail
            && ((transactionDetail.debitAmount - transactionDetail.realAmount) > 0)
          ) ? transactionDetail.debitAmount : (
              (typeRefund === 1)
                ? (transactionDetail && transactionDetail.debitAmount) : initAmount
            ),
        amountRefund: (
          transactionDetail
          && ((transactionDetail.debitAmount - transactionDetail.realAmount) > 0)
        ) ? (transactionDetail.realAmount || 0)
          : (transactionDetail && transactionDetail.refundBeans
          && (transactionDetail.refundBeans.length > 0)) ? initAmount
            : (
              (typeRefund === 1)
                ? (transactionDetail && transactionDetail.realAmount ? transactionDetail.realAmount : 0)
                : (initAmount - (transactionDetail
              && (transactionDetail.debitAmount - transactionDetail.realAmount)))
            ),
        note: description,
        status: JSON.parse(window.atob(window.localStorage.getItem('STATUS_TRANSACTION_DETAIL'))),
        online: JSON.parse(window.atob(window.localStorage.getItem('ONLINE_TRANSACTION_DETAIL'))),
      ...
      data.amountRefund = Number(data.amountRefund);
      if (
        data.initAmount && data.amountRefund && (parseInt(data.initAmount, 10) >= 1000)
        && (this.form.getChildContext()._errors.length === 0)
      ) {
        createTransactionRefund(data);
      }
    }
  };
  ...
  render() {
    const {
      ...
      listBanks,
      validateForm,
      checkInitAmountSmallerDebitAmount,
      initAmount,
      typeRefund,
    } = this.state;
    const {
    ...
    const arrTypeRefund = [
      { value: 1, label: 'Hoàn toàn phần' },
      { value: 2, label: 'Hoàn một phần' },
    ];
          ...
          <CardBody>
              ...
              <div className="col-md-6">
                ...
                <div className="widget-box transparent">
                  ...
                  <Collapse isOpen={isOpenInfoCustom} className="show-information">
                    <Form className="widget-box transparent" ref={(addTransitionRefund) => { this.form = addTransitionRefund; }}>
                      <Card>
                        <CardBody>
                          <div className="row">
                            <FormGroup row className="col-md-12">
                                ...
                                ? (
                                  <>
                                    <FormGroup row className="col-md-12">
                                      ...
                                      <Col md="6">
                                        <Select
                                          value={{ value: 1, label: 'Hoàn toàn phần' }}
                                          isDisabled
                                        />
                                      </Col>
                                    </FormGroup>
                                    <FormGroup row className="col-md-12">
                                      ...
                                      <Col md="6">
                                        <NumberFormat
                                          ...
                                          maxLength={10}
                                          value={
                                            transactionDetail && transactionDetail.debitAmount
                                          }
                                          disabled
                                        />
                                      </Col>
                                  ...
                                  // so tien truoc khuyen mai = so tien sau khuyen mai
                                  <>
                                    {
                                      transactionDetail && transactionDetail.refundBeans
                                      && (transactionDetail.refundBeans.length > 0) ? (
                                        <>
                                          <FormGroup row className="col-md-12">
                                            ...
                                            <Col md="6">
                                              <Select
                                                value={{ value: 2, label: 'Hoàn một phần' }}
                                                isDisabled
                                              />
                                            </Col>
                                          </FormGroup>
                                          <FormGroup row className="col-md-12">
                                            ...
                                            <Col md="6">
                                              <NumberFormat
                                                ...
                                                thousandSeparator
                                                maxLength={10}
                                                onChange={this.handleChangeMoneyRequiredRefund}
                                                value={initAmount}
                                                // decimalSeparator=""
                                              />
                                              {
                                              ...
                                              {
                                                validateForm && checkRequiredSelectInput(initAmount,
                                                  'Số tiền yêu cầu hoàn')
                                              }
                                              {
                                                validateForm
                                                && (initAmount && parseInt(initAmount, 10) < 1000)
                                                && (
                                                  <span className="text-danger">Số tiền phải lớn hơn hoặc bằng 1000</span>
                                                )
                                              }
                                            </Col>
                                          </FormGroup>
                                          ...
                                          <>
                                            <FormGroup row className="col-md-12">
                                              ...
                                              <Col md="6">
                                                <Select
                                                  options={arrTypeRefund}
                                                  onChange={this.onChangeTypeRefund}
                                                  defaultValue={{ value: 1, label: 'Hoàn toàn phần' }}
                                                />
                                                {
                                                  checkInitAmountSmallerDebitAmount && (
                                                    <span className="text-danger">
                                                  ...
                                                  <FormGroup row className="col-md-12">
                                                    ...
                                                    <Col md="6">
                                                      <NumberFormat
                                                        ...
                                                        maxLength={10}
                                                        onChange={
                                                          this.handleChangeMoneyRequiredRefund
                                                        }
                                                        value={initAmount}
                                                        // decimalSeparator=""
                                                      />
                                                      {
                                                        validateForm
                                                        && checkRequiredSelectInput(initAmount,
                                                          'Số tiền yêu cầu hoàn')
                                                      }
                                                      {
                                                        validateForm
            ...
            <Row>
              <Col className="text-center btn-search mt-3 mb-5">
                <div>
                  {numberRefund < 3 && typeRefund !== 2 ? (
                    <Button onClick={() => this.createRefundTransaction()} onKeyPress={history.goBack} className="btn btn-primary btn btn-secondary mr-3">
                      Khởi tạo giao dịch hoàn tiền
                    </Button>
                  ) : null}
...
const mapStateToProps = (state, ownProps) => ({
  ...
  responseServer: state.Transaction.responseServer,
  response: state.Transaction.response,
});

const mapDispatchToProps = (dispatch) => ({
  getTransactionDetail: (data) => dispatch(getTransactionDetail(data)),
  createTransactionRefund: (data) => dispatch(createTransactionRefund(data)),
});

AddTransitionRefund.defaultProps = {
  getTransactionDetail: () => {},
  createTransactionRefund: () => {},
  history: {},
};

AddTransitionRefund.propTypes = {
  getTransactionDetail: PropTypes.func,
  createTransactionRefund: PropTypes.func,
  history: PropTypes.object,
};

export default connect(mapStateToProps, mapDispatchToProps)(withRouter(AddTransitionRefund));
