...
class FillMerchant extends Component {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      ...
      let newData = {};
      if (searchParams && data.terminalListDenied) {
        newData = {
          data: {
            ...data.terminalListDenied.data,
            terminalId: data.terminalListDenied.data.terminalId.value || '',
            merchantId: data.terminalListDenied.data.merchantId.merchantId || '',
            terminalBranchId: data.terminalListDenied.data.terminalBranchId.value || '',
            levelTerminal: data.terminalListDenied.data.levelTerminal.value || '',
            deniedId: data.terminalListDenied.data.deniedId.value || 0,
            deniedUser: data.terminalListDenied.data.deniedUser.label ? data.terminalListDenied.data.deniedUser.label === 'Tất cả' ? '' : data.terminalListDenied.data.deniedUser.label : '',
          },
          fromRow: data.terminalListDenied.fromRow,
          pageSize: data.terminalListDenied.pageSize,
          fromDate: data.terminalListDenied.fromDate,
          toDate: data.terminalListDenied.toDate,
        };
      }
      ...
      if (searchParams && data.terminalListDenied) {
        this.setState({
          data: data.terminalListDenied.data,
          fromDate: new Date(moment(data.terminalListDenied.fromDate, 'DD/MM/YYYY 00:00:00')),
          toDate: new Date(moment(data.terminalListDenied.toDate, 'DD/MM/YYYY 00:00:00')),
          fromRow: data.terminalListDenied.fromRow,
          pageSize: data.terminalListDenied.pageSize,
        });
        ...
        updateListMerchantBranch.push(
          {
            ...
            merchantCode: listMerchantBranch[i] && listMerchantBranch[i] && listMerchantBranch[i].merchantCode,
    ...
    if (listTerminalPlace) {
      for (let i = 0; i < listTerminalPlace.length; i += 1) {
        updatelistTerminalPlace.push(
          {
            placeId: listTerminalPlace[i] && listTerminalPlace[i].id,
            value: listTerminalPlace[i] && listTerminalPlace[i].id,
            merchantCode: listTerminalPlace[i] && listMerchantBranch[i] && listTerminalPlace[i] && listMerchantBranch[i].merchantCode,
            label: `${listTerminalPlace[i] && listTerminalPlace[i].merchantCode} - ${listTerminalPlace[i] && listTerminalPlace[i].terminalName}`,
          },
        );
      }
    }
