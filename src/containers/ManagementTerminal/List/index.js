...
class TerminalList extends Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      ...
      if (searchParams && searchParams.terminalList) {
        this.setState({
          currentPage: searchParams.terminalList.fromRow / searchParams.terminalList.pageSize + 1,
          perPage: searchParams.terminalList.pageSize,
        });
      }
    }, 0);
  }
  ...
  handlePageChange = (pageNumber) => {
    ...
    if (
      ...
      this.setState({
        ...
        storedData = {
          ...(storedData && storedData.terminalList) || defaultData,
    ...
    return (
      <>
        <div className="animated fadeIn fee-table" style={{ background: 'white' }}>
          <Card>
            <CardHeader>
              <span className="text-bold">{Constants.TerminalList.terminalList}</span>
            </CardHeader>
            <CardBody>
              <FillMerchant
              ...
              />
              <TableTerminal
                tData={listTerminalTable && listTerminalTable.data}
                perPage={perPage}
                currentPage={currentPage}
              />
