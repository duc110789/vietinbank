...
import {
  getDetailTranBusinessSuccess,
  getExcelFileReportBusiness,
...
class FillReport extends Component {
  ...
  componentDidMount() {
    const {
      getDetailTranBusinessSuccess, getMerchantCodeName,
    } = this.props;
    const sendData = {
    ...
    getDetailTranBusinessSuccess(sendData);
  }
        ...
        updateListMerchantBranch.push(
          {
            ...
            label: `${listMerchantBranch[i] && listMerchantBranch[i].merchantCode} - ${listMerchantBranch[i] && listMerchantBranch[i].terminalName}`,
          },
        );
      }
    }
        ...
        updatelistTerminalPlace.push(
          {
            ...
            merchantCode: listTerminalPlace[i] && listTerminalPlace[i].merchantCode,
  ...
  componentDidUpdate(prevProps) {
    const { exportReportFileBusiness } = this.props;
    if (prevProps.exportReportFileBusiness !== exportReportFileBusiness) {
      window.open(exportReportFileBusiness, 'blank');
    }
  }
  ...
  selectMerchantBranch = (e) => {
    ...
    if (e.value === '00') {
      ...
      this.setState({
        ...
        data: {
          ...this.state.data,
          terminalIdBrand: e.value === '' ? '' : `${e.branchId}`,
        },
    ...
    if (listQrChannel) {
      for (let i = 0; i < listQrChannel.length; i += 1) {
        updateListQrChannel.push({
          value: listQrChannel[i] && listQrChannel[i].code,
          label: `${listQrChannel[i] && listQrChannel[i].description}`,
        });
      }
    }
  ...
  onHandleSearch = () => {
    ...
    getDetailTranBusinessSuccess(dataSearch);
  };
  ...
  exportFile = () => {
    ...
    getExcelFileReportBusiness(dataExport);
  }
...
const mapStateToProps = (state) => ({
  ...
  exportReportFileBusiness: state.statisticalReport.exportReportFileBusiness,
...
const mapDispatchToProps = (dispatch) => ({
  ...
  getExcelFileReportBusiness: (data) => dispatch(getExcelFileReportBusiness(data)),
...
FillReport.propTypes = {
  getDetailTranBusinessSuccess: PropTypes.func,
  getMerchantCodeName: PropTypes.func,
  listMerchantCodeName: PropTypes.array,
  getExcelFileReportBusiness: PropTypes.func,
  exportReportFileBusiness: PropTypes.object,
