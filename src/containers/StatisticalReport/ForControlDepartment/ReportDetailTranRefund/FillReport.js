...
import {getDetailTransactionRefund, getExcelFileReportRefund,} from '../../../../store/actions/StatisticalReport/index';
...
class FillReport extends Component {
  ...
  componentDidMount() {
    const {
      getDetailTransactionRefund, getMerchantCodeName,
    } = this.props;
    const sendData = {
    ...
    getDetailTransactionRefund(sendData);
  }
  ...
  componentDidUpdate(prevProps) {
    const { exportReportFileRefund } = this.props;
    if (prevProps.exportReportFileRefund !== exportReportFileRefund) {
      window.open(exportReportFileRefund, 'blank');
    }
  }
  ...
  selectMerchantBranch = (e) => {
    ...
    if (e.value === '00') {
      ...
      this.setState({
        ...
        data: {
          ...this.state.data,
          terminalIdBrand: `${e.value}`,
        },
      }, () => getTerminalPlace({ merchantCode: e.value, status: '1', terminalBranchId: `${e.value}` }));
    }
  };
  ...
  onHandleSearch = () => {
    ...
    getDetailTransactionRefund(dataSearch);
  };

  exportFile = () => {
    const {
      getExcelFileReportRefund,
    } = this.props;
    const {
      fromDate,
      toDate,
    ...
    if (fromDate.getTime() > toDate.getTime()) {
      ...
      getExcelFileReportRefund(dataExport);
    }
  }
...
const mapStateToProps = (state) => ({
  ...
  exportReportFileRefund: state.statisticalReport.exportReportFileRefund,
...
const mapDispatchToProps = (dispatch) => ({
  ...
  getExcelFileReportRefund: (data) => dispatch(getExcelFileReportRefund(data)),
...
FillReport.propTypes = {
  getDetailTransactionRefund: PropTypes.func,
  getMerchantCodeName: PropTypes.func,
  listMerchantCodeName: PropTypes.array,
  getExcelFileReportRefund: PropTypes.func,
  exportReportFileRefund: PropTypes.object,
